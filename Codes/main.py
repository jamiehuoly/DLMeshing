...
if GENERATE_FINE_MESH:
    ...
    LC_FINE = 2.5  # fine section
    LC_COARSE = 6  # coarse section

    # let 30% coarse, 70% fine
    bbox = gmsh.model.getBoundingBox(-1, -1)
    ymin, ymax = bbox[1], bbox[4]
    zmin, zmax = bbox[2], bbox[5]
    yaxis_refine_cutoff = ymax + (ymin - ymax) * 0.3 # the y-coordinate is inversed
    zaxis_refine_cutoff = zmin + (zmax - zmin) * 0.3
